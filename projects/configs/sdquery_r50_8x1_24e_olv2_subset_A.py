_base_ = []
custom_imports = dict(imports=['projects.bevformer', 'projects.sdquery'])
...
dataset_type = 'OpenLaneV2_subset_A_SDQuery_Dataset'
...
model = dict(
            ...
            decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    operation_order=('self_attn', 'norm', 'cross_attn', 'norm',
                                     'ffn', 'norm')))),
        bbox_coder=dict(type='SDQueryPseudoCoder'),
        loss_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
    ...
    train_cfg=dict(
        lane=dict(
            assigner=dict(
                type='SDQueryHungarianAssigner3D',
...
train_pipeline = [
    ...
    dict(type='GenerateLaneMask', points_num=method_para['n_points'], bev_h=bev_h_, bev_w=bev_w_),
    dict(type='CustomFormatBundle3DLane', class_names=class_names),
    dict(type='CustomCollect3D', keys=[
        'img','map_graph', 'onehot_category', 'gt_lanes_3d', 'gt_lane_labels_3d', 'gt_lane_adj',
        'gt_instance_masks'])
]
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_subset_A_train_sdquery.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_subset_A_val_sdquery.pkl',
