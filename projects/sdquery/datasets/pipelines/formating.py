...
class CustomFormatBundle3DLane(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        if 'map_graph' in results:
            # deciding between: num_polylines x max_num_vectors x 3 (points) or num_polylines x 15 (curves)
            results['map_graph'] = DC(to_tensor(results['map_graph']))
        if 'onehot_category' in results:
            results['onehot_category'] = DC(to_tensor(results['onehot_category']))
