...
# from .coder import bezier_prediction_decode
from . import decoder
...
class OpenLaneV2_subset_A_SDQuery_Dataset(Custom3DDataset):
    ...
    def __init__(self,
                ...
                #  decoding_function='bezier_prediction_decode',
                ):
    ...
    def evaluate(self, 
        ...
        metric_results = {}
        if not visualization:  # compute metrics only if visualization is 
            if evaluate_normal:
            ...
            if evaluate_by_dist:
                for key, val in evaluate_by_distance(ground_truth=self.ann_file, predictions=pred_dict).items():
                    for k, v in val.items():
                        metric_results[k if k != 'score' else key] = v
            if evaluate_by_int:
                for key, val in evaluate_by_intersection(ground_truth=self.ann_file, predictions=pred_dict).items():
                    for k, v in val.items():
                        metric_results[k if k != 'score' else key] = v
    ...
    def format_preds(self, results):
        ...
        for index, result in enumerate(results):
            ...
            # lanes = lanes.numpy()
            lanes = self.decoding_function(lanes, **self.decoding_para)
...
@DATASETS.register_module()
class OpenLaneV2SDMapDataset(OpenLaneV2SubsetADataset):
    ...
    def get_data_info(self, index):
        ...
        # data/OpenLane-V2/sd_map/train/00492/315970269249927220.png
...
@DATASETS.register_module()
class OpenLaneV2GraphicalSDMapDataset(OpenLaneV2SubsetADataset):
    ...
    def __init__(self,
        ...
        #     'residential': 5, 
        #     'service': 6, 
        #     'pedestrian': 2,  # set to be sidewalk
        #     'bus_way': 7, 
        #     'other': 8,
        # }
